        ...
        for ent in doc.ents:
            ...
            if self._is_valid_entity(ent.text, entity_type, ent):
                entities.append({
                    ...
                    'confidence': getattr(ent, 'confidence', 1.0) if hasattr(ent, '_') else 1.0
                })
...
def _is_valid_entity(self, entity_text: str, entity_type: str, spacy_ent) -> bool:
        ...
        # Filter out words functioning primarily as verbs or auxiliaries
...
def _detect_geographical_context(self, text: str, entities: List[Dict[str, Any]]) -> List[str]:
        ...
        for entity in entities:
            entity_type = entity.get('type', entity.get('label', 'UNKNOWN'))
            if entity_type in ['GPE', 'LOCATION'] and entity.get('text', '').lower() not in [c.lower() for c in context_clues]:
                context_clues.append(entity.get('text', '').lower())
...
def _try_contextual_geocoding(self, entity, context_clues):
        ...
        search_variations = [entity.get('text', '')]
        ...
        for context in context_clues:
            ...
            for variant in context_variants:
                search_variations.extend([
                    f"{entity.get('text', '')}, {variant}",
                    f"{entity.get('text', '')} {variant}",
                    f"ancient {entity.get('text', '')} {variant}" if 'ancient' not in entity.get('text', '').lower() else f"{entity.get('text', '')} {variant}"
                ])
...
def _try_python_geocoding(self, entity):
        ...
        try:
            ...
            for name, geocoder in geocoders:
                try:
                    location = geocoder.geocode(entity.get('text', ''), timeout=15)
                    if location:
                        entity['latitude'] = location.latitude
                        entity['longitude'] = location.longitude
                        entity['location_name'] = location.address
...
def _try_openstreetmap(self, entity):
        ...
        try:
            ...
            print(f"OpenStreetMap geocoding failed for {entity.get('text', '')}: {e}")
            pass
...
def _try_aggressive_geocoding(self, entity):
        ...
        search_variations = [
            entity_text,
            f"{entity_text}, Greece",  # Default to Greece for ancient names
            f"{entity_text}, Turkey",  # Many ancient places are now in Turkey
            f"{entity_text}, Italy",   # Or Italy
            f"ancient {entity_text}",
            f"{entity_text} ancient city",
            f"{entity_text} archaeological site",
        ...
        for entity in entities:
            try:
                ...
                search_terms = [
                    ...
                    f"{entity_text} city" if entity_type in ['GPE', 'LOCATION'] else entity_text
                ]
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get('search') and len(data['search']) > 0:
                            ...
                            for result in data['search']:
                                ...
                                if (result['label'].lower() == entity_text.lower() or
                                    entity_text.lower() in result['label'].lower()):
        ...
        for entity in entities:
            ...
            try:
                ...
                search_terms = [
                    ...
                    f"{entity_text} ancient",
                    f"{entity_text} city" if entity_type in ['GPE', 'LOCATION'] else entity_text,
                    f"{entity_text} (ancient city)" if entity_type in ['GPE', 'LOCATION'] else entity_text
                ]
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get('query', {}).get('search'):
                            ...
                            for result in data['query']['search']:
                                ...
                                # Prefer exact matches or close matches
                                if (entity_text.lower() in page_title.lower() or
                                    any(word in page_title.lower() for word in entity_text.lower().split())):
                ...
                print(f"Wikipedia linking failed for {entity.get('text', '')}: {e}")
                pass
                    ...
                    if response.status_code == 200:
                        ...
                        for pattern in patterns:
                            ...
                            for url_path, link_text in matches:
                                if (entity_text.lower() in link_text.lower() or 
                                    link_text.lower() in entity_text.lower() or
                                    any(word in link_text.lower() for word in entity_text.lower().split())):
...
class StreamlitEntityLinker:
    ...
    def process_text(self, text: str, title: str):
        ...
        if text_hash == st.session_state.get('last_processed_hash'):
        ...
        with st.spinner("Processing text and extracting entities..."):
            try:
                ...
                debug_text.text(f"Found {len(entities)} initial entities: {[e.get('text', 'Unknown') for e in entities[:5]]}...")
    ...
    def create_highlighted_html(self, text: str, entities: List[Dict[str, Any]]) -> str:
        ...
        for entity in sorted_entities:
            ...
            entity_type = entity.get('type', entity.get('label', 'UNKNOWN'))
            color = colors.get(entity_type, '#E7E2D2')
            ...
            border_style = ""
            if has_pleiades:
                url = html_module.escape(entity.get("pleiades_url", ""))
    ...
    def render_pelagios_entity_card(self, entity: Dict):
        ...
        with col1:
            st.markdown(f"### **{entity.get('text', 'Unknown')}**")
            entity_type = entity.get('type', entity.get('label', 'UNKNOWN'))
    ...
    def create_enhanced_jsonld_export(self, entities: List[Dict[str, Any]]) -> Dict:
        ...
        for entity in entities:
            entity_type = entity.get('type', entity.get('label', 'UNKNOWN'))
            entity_data = {
                "@type": self._get_schema_type(entity_type),
                "name": entity.get('text', 'Unknown'),
                "entityType": entity_type,
                "startOffset": entity.get('start', 0),
...
class PelagiosIntegration:
    ...
    def enhance_entities_with_pelagios(self, entities: List[Dict]) -> List[Dict]:
        ...
        for entity in entities:
            # FIXED: Use safe dictionary access instead of assuming entity structure
    ...
    def export_to_recogito_format(self, text: str, entities: List[Dict], 
        ...
        for idx, entity in enumerate(entities):
            ...
            entity_type = entity.get('type', entity.get('label', 'UNKNOWN'))
            if entity_type in ['GPE', 'LOCATION', 'FACILITY', 'ADDRESS']:
                annotation = {
        ...
        for entity in sorted_entities:
            ...
            if start > current_pos:
                if len(p) == 0 and p.text is None:
                    p.text = text[current_pos:start]
                else:
                    # Add to tail of last element
                    if len(p) > 0:
                        if p[-1].tail is None:
                            p[-1].tail = text[current_pos:start]
                        else:
                            p[-1].tail += text[current_pos:start]
            
            # Create place name element based on entity type
            if entity_type in ['GPE', 'LOCATION']:
            ...
            place_elem.text = entity.get('text', '')
            
            # Add attributes
            if entity.get('pleiades_id'):
                place_elem.set('ref', entity.get('pleiades_url', ''))
            elif entity.get('wikidata_url'):
                place_elem.set('ref', entity.get('wikidata_url', ''))
